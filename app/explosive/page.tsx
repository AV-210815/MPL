...
export default async function ExplosivePage({ searchParams }: { searchParams: Promise<{ format?: string }> }) {
  const { format } = await searchParams;
  const rows = await getExplosiveLeaderboard(format);
  ...
  return (
    <div className="relative -mt-6 -mx-4 px-4 pt-0 overflow-hidden">
      ...
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        ...
        <div className="absolute bottom-20 right-1/3 w-[200px] h-[200px] rounded-full bg-yellow-600/8 blur-[70px]" />
      </div>
        ...
        <div className="relative mb-6 rounded-2xl border border-red-500/20 bg-gradient-to-r from-red-500/10 via-orange-500/8 to-transparent overflow-hidden">
          ...
          <div className="px-5 py-4 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="font-black text-white text-2xl leading-tight" style={{ fontFamily: "var(--font-rajdhani)" }}>
                {rows[0].name}
              </p>
              <div className="flex items-center gap-3 mt-0.5">
                ...
                <span className="text-sm text-gray-400">
                  <span className="text-blue-300 font-semibold">{rows[0].fours}</span> fours
                </span>
                <span className="text-gray-700">·</span>
                <span className="text-sm text-gray-400">
                  <span className="text-purple-300 font-semibold">{rows[0].sixes}</span> sixes
                </span>
              </div>
            </div>
          </div>
      ...
      <div className="relative mb-6">
        <Suspense>
          <FormatFilter basePath="/explosive" />
        </Suspense>
      </div>
