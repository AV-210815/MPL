...
export default async function SlugEditMatchPage({ params }: Props) {
  ...
  const [match, players] = await Promise.all([
    prisma.match.findUnique({
      where: { id: matchId, apartmentId: apartment.id },
      include: {
        members: true,
    ...
    prisma.player.findMany({ where: { apartmentId: apartment.id }, orderBy: { name: "asc" } }),
  ]);
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-black text-white">Edit {match.label ?? `Match #${match.id}`}</h1>
      </div>
      <MatchForm players={players} matchId={matchId} initial={initial} redirectTo={`/${slug}/matches`} />
    </div>
  );
}
