    ...
    <div className="relative -mt-6 -mx-4 px-4 pt-0 overflow-hidden">
      ...
      <div className="relative max-w-xl space-y-4 pb-10">
        ...
        <form onSubmit={addPlayer}
          ...
          <p className="font-black text-white text-sm uppercase tracking-wider" style={{ fontFamily: "var(--font-rajdhani)" }}>
            Add New Player
          </p>
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-green-500/50 focus:bg-white/8 transition-all"
            />
            <button type="submit" disabled={saving || !newName.trim()}
              className="px-5 py-2.5 bg-green-600 hover:bg-green-500 disabled:opacity-40 disabled:cursor-not-allowed text-white text-sm font-bold rounded-xl transition-all shadow-[0_0_16px_rgba(22,163,74,0.3)] hover:shadow-[0_0_24px_rgba(22,163,74,0.5)]">
              {saving ? "…" : "Add"}
            </button>
          </div>
          {error && (
                  ...
                  {editId === p.id ? (
                    <>
                      <input
                        ...
                        className="flex-1 bg-white/8 border border-blue-500/30 rounded-lg px-3 py-1.5 text-sm text-white focus:outline-none focus:border-blue-400/60"
                      />
                      <button onClick={() => saveEdit(p.id)}
                        className="text-xs text-green-400 hover:text-green-300 font-bold px-2 transition-colors">Save</button>
                      <button onClick={() => setEditId(null)}
                        className="text-xs text-gray-600 hover:text-gray-400 px-2 transition-colors">Cancel</button>
                    </>
                  ) : (
                    <>
                      <div className="flex-1 min-w-0">
                        <span className="text-white font-semibold block truncate" style={{ fontFamily: "var(--font-rajdhani)", fontSize: "1.05rem" }}>
                          {p.name}
                        </span>
                        {p.photo && (
                      ...
                      <button onClick={() => triggerPhotoUpload(p.id)}
                        disabled={isUploading}
                        className="text-xs text-green-400 hover:text-green-300 font-medium px-2.5 py-1.5 rounded-lg hover:bg-green-500/10 transition-all disabled:opacity-40">
                        {isUploading ? "…" : p.photo ? "📷 Change" : "📷 Photo"}
                      </button>
