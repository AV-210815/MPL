...
interface BattingProfile { innings: number; runs: number; avg: number | null; sr: number; hundreds: number; fifties: number; fours: number; sixes: number; bestScore: { runs: number; notOut: boolean } | null }
interface BowlingProfile { wickets: number; maidens: number; overs: number; runsConceded: number; economy: number; avg: number | null; bestFigures: { wickets: number; runsConceded: number } | null }
interface Badge { icon: string; label: string; desc: string }
interface Profile { id: number; name: string; photo?: string | null; matches: number; batting: BattingProfile; bowling: BowlingProfile; recentForm: { runs: number; notOut: boolean }[]; badges: Badge[] }

function fmtAvg(v: number | null) { if (v === null) return "∞"; if (v === 0) return "-"; return v.toFixed(1); }
...
export default function SlugProfilesPage() {
  ...
  return (
    <div className="relative -mt-6 -mx-4 px-4 pt-0 overflow-hidden">
      ...
      <div className="relative pt-10 pb-8">
        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
          <div className="flex-1">
            ...
            <p className="text-gray-500 mt-2 text-sm" style={{ fontFamily: "var(--font-rajdhani)", fontWeight: 600 }}>Career milestones from all matches</p>
          </div>
      ...
      {loading ? (
        <div className="text-center py-20 text-gray-600"><div className="text-4xl mb-3 animate-pulse">📊</div><p>Loading profiles…</p></div>
      ) : loadError ? (
        ...
        <div className="text-center py-20 text-gray-600 border border-white/5 rounded-2xl bg-white/[0.02]">
          <div className="text-4xl mb-3">📭</div><p className="font-semibold">No players yet.</p>
        </div>
      ) : (
            ...
            return (
              <div key={p.id}
                ...
                <div className={`relative bg-gradient-to-br ${theme.banner} px-5 pt-6 pb-16 overflow-hidden`}>
                  ...
                  <div className="absolute right-3 top-1 text-[7rem] font-black text-white/10 leading-none select-none" style={{ fontFamily: "var(--font-bebas)" }}>{idx + 1}</div>
                  <p className="relative text-[10px] uppercase tracking-[0.2em] text-white/60 font-semibold mb-1" style={{ fontFamily: "var(--font-oswald)" }}>{p.matches} match{p.matches !== 1 ? "es" : ""}</p>
                  <p className="relative text-2xl font-black text-white leading-tight" style={{ fontFamily: "var(--font-rajdhani)" }}>{p.name}</p>
                </div>
                <div className="relative px-5">
                  <div className="-mt-10 mb-3 flex items-end justify-between">
                    {p.photo ? (
                      <div className="w-20 h-20 rounded-full overflow-hidden ring-4 ring-[#0d0d1a] shadow-2xl" style={{ boxShadow: `0 0 24px ${theme.glow}` }}>
                  ...
                  <div className="flex gap-2 flex-wrap mb-4">
                    {p.batting.hundreds > 0 && <span className="px-2.5 py-1 rounded-full bg-yellow-500/15 border border-yellow-500/25 text-yellow-300 text-[10px] font-bold uppercase tracking-wider">{p.batting.hundreds} × 💯</span>}
                    {p.batting.fifties > 0 && <span className="px-2.5 py-1 rounded-full bg-amber-500/15 border border-amber-500/25 text-amber-300 text-[10px] font-bold uppercase tracking-wider">{p.batting.fifties} × 50</span>}
                    {p.batting.sixes > 0 && <span className="px-2.5 py-1 rounded-full bg-red-500/15 border border-red-500/20 text-red-300 text-[10px] font-bold uppercase tracking-wider">{p.batting.sixes} × 6️⃣</span>}
                    {p.batting.bestScore && <span className="px-2.5 py-1 rounded-full bg-orange-500/15 border border-orange-500/25 text-orange-300 text-[10px] font-bold tracking-wider">HS {p.batting.bestScore.runs}{p.batting.bestScore.notOut ? "*" : ""}</span>}
                    {p.bowling.bestFigures && <span className="px-2.5 py-1 rounded-full bg-purple-500/15 border border-purple-500/25 text-purple-300 text-[10px] font-bold tracking-wider">BB {p.bowling.bestFigures.wickets}/{p.bowling.bestFigures.runsConceded}</span>}
                  </div>
                ...
                {isOpen && (
                  <div className="border-t border-white/8 px-5 pb-5 pt-4 space-y-5">
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-orange-400 mb-2" style={{ fontFamily: "var(--font-oswald)" }}>🏏 Batting</p>
                    ...
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-purple-400 mb-2" style={{ fontFamily: "var(--font-oswald)" }}>🎯 Bowling</p>
